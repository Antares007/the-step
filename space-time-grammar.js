...
function bnf_compiler(...symbols) {
  ...
  function time_terminal    (x,t) { write(JSON.stringify(x)), next_time(t) }
  function time_branch      (s,t) { if(symbols.indexOf(s) === -1)
                                      symbols.push(s)
                                    write(s.name), next_time(t) }
  function next_time_termin (x,t) { write(' '), time_terminal(x, t) }
  function next_time_branch (s,t) { write(' '), time_branch(s, t) }

  function space_dot        (   ) { write('.\n'),             fetch() }
  function space_termin     (x,s) { write(JSON.stringify(x)), next_space(s)}
  ...
  function next_space_termin(x,s) { write(', '), space_termin(x, s) }
  function next_space_branch(s,t) { write(', '), space_branch(s, t) }

  function init_time (s) { s(time_dot, time_terminal, time_branch) }
